...
*/
/**Programmet vil bare løse den første linjen, er litt lite detaljert.
 * @author Nicklash
 */
class SudokuBuffer {
			...
			while (sc.hasNextLine()){
				String linje = sc.nextLine(); 
					...
					for(int j = 0; j<d; j++){
						...
						if(linje.charAt(j) != '.'){
							ruter[i][j].setVerdi(Character.getNumericValue(linje.charAt(j)));
						}
...
class settRute extends hovedRute {
/*	private void setVerdi(int v){
		this.verdi = v;
	}*/
}
